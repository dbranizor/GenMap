...
export abstract class AbstractLayer<T> {
    ...
    */
    ...
    private handleShape(shape: T) {
        ...
        if (!this._shape) {
            this._shape = shape;
            console.log('dingo this shape', this.render, this);
            this.render();
        } else {
            this.update();
        }
    ...
    private getShape() {
        this.renderer.renderer.mapObjectLayers$.subscribe(s => {
            console.log('dingo layers', s);
            if (s && s.length) {
                console.log('dingo layers mt', this.mapToolLayer);
                this.mapToolLayer = s.find(lyr => lyr.id === this.id);
                console.log('dingo layers mtt', this.mapToolLayer);
            }
